 ...
 *
 * Las fórmulas se devuelven como HTML (fracciones apiladas, raíces con
 * barra) generado con QuadApp.data.FormatoMatematico; no recalcula nada
 * por su cuenta, solo explica los valores ya obtenidos por
...
(function (ns) {
  function generar(funcion, resultado) {
    ...
    pasos.push({
      ...
      sustitucion: `a = ${fmt(a)} ${a > 0 ? '(a &gt; 0)' : '(a &lt; 0)'}`,
    ...
    pasos.push({
      titulo: 'Vértice',
      formulaGeneral: `x\u1D65 = ${frac('-b', '2a')}&nbsp;&nbsp;&nbsp;y\u1D65 = f(x\u1D65)`,
      sustitucion: `x\u1D65 = ${frac(`-(${fmt(b)})`, `2\u00B7(${fmt(a)})`)} = ${fmt(r.verticeX)}`,
      resultado: `V (${fmt(r.verticeX)}, ${fmt(r.verticeY)})`,
      interpretacion: `Se evalúa f(${fmt(r.verticeX)}) para obtener y\u1D65 = ${fmt(r.verticeY)}.`,
    });
    ...
    pasos.push({
      ...
      sustitucion: `Δ = (${fmt(b)})² - 4\u00B7(${fmt(a)})\u00B7(${fmt(c)}) = ${fmt(r.discriminante)}`,
    ...
    const formulaX = `x = ${frac(`-b ± ${raiz('Δ')}`, '2a')}`;
    let sustitucionX;
    let resultadoX;
    if (r.tieneRaicesReales) {
      if (r.raiz1 === r.raiz2) {
        sustitucionX = `x = ${frac(`-(${fmt(b)})`, `2\u00B7(${fmt(a)})`)} = ${fmt(r.raiz1)}`;
        resultadoX = `(${fmt(r.raiz1)}, 0)`;
      } else {
        sustitucionX = `x = ${frac(`-(${fmt(b)}) ± ${raiz(fmt(r.discriminante))}`, `2\u00B7(${fmt(a)})`)}`;
        resultadoX = `(${fmt(r.raiz1)}, 0)  y  (${fmt(r.raiz2)}, 0)`;
      }
    } else {
      sustitucionX = `${raiz('Δ')} no existe en los reales porque Δ &lt; 0`;
      resultadoX = 'No hay intersección con el eje X';
    }
    pasos.push({
      titulo: 'Intersección con el eje X',
      formulaGeneral: formulaX,
