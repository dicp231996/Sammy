...
(function (ns) {
  ...
  class GraphPanel {
    ...
    _ajustarTamano() {
      ...
      // El tamaño del panel cambió: si ya había una función graficada,
      // recalculamos el rango para que la escala 1:1 siga siendo correcta.
    ...
    /**
     * Ajusta xMin/xMax/yMin/yMax para que 1 unidad en X ocupe exactamente
     * los mismos píxeles que 1 unidad en Y (aspecto 1:1), expandiendo el
     * eje que "sobra" espacio en vez de recortar el que falta, para no
     * perder ninguno de los puntos que ya decidimos que debían verse.
     */
    _igualarEscala() {
      ...
      const rangoX = this.xMax - this.xMin;
      const rangoY = this.yMax - this.yMin;
      if (!(rangoX > 0) || !(rangoY > 0)) return;

      const escala = Math.min(width / rangoX, height / rangoY);

      const centroX = (this.xMin + this.xMax) / 2;
      const centroY = (this.yMin + this.yMax) / 2;

      const nuevoRangoX = width / escala;
      const nuevoRangoY = height / escala;

      this.xMin = centroX - nuevoRangoX / 2;
      this.xMax = centroX + nuevoRangoX / 2;
      this.yMin = centroY - nuevoRangoY / 2;
      this.yMax = centroY + nuevoRangoY / 2;
    }
